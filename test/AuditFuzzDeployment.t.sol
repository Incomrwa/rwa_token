...
contract AuditFuzzDeployment is Test {
    ...
    function Deploy(uint64 startTimestamp, uint64 cliffDuration, uint64 totalDuration, uint256 amount) public {
        deployer = msg.sender;
    ...
    function test_StartTimeInPassed(uint64 _cliffDuration, uint64 _totalDuration, uint256 _amount) public {
        ...
        Deploy(uint64(block.timestamp - 1 days), _cliffDuration, _totalDuration, _amount);
        uint256 amount = vesting.vestedAmount(address(token), (uint64(block.timestamp) + _totalDuration + 1 days));
